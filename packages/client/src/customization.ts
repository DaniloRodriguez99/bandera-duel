import { CHARACTER_SKINS, CLASSES, SKILLS, SKILL_SLOTS, activePreset, defaultCustomization, validCustomization, type BindableAction, type CharacterCustomization, type ClassId, type PhysicalBinding, type SkillId, type SkillSlot } from '@bandera/shared';
...
export function mountCharacterCustomization(root:HTMLElement, initial:CharacterCustomization, onChange:(classId:ClassId,value:CharacterCustomization)=>void){
  ...
  const render=()=>{
    ...
    root.innerHTML=`<div class="custom-shell" role="dialog" aria-modal="true" aria-labelledby="custom-title">
      ...
      <section class="custom-center">${tab==='skins'?`<div class="skin-grid">${skins.map(item=>`<button data-skin="${item.id}" aria-pressed="${item.id===value.selectedSkin}" style="--swatch:${item.cloth};--accent:${item.accent}"><i></i><span>${item.name}</span></button>`).join('')}</div>`:`<div class="skill-tree">${skillBranches()}</div>`}</section>
