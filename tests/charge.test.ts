...
it('carga mantenida no dispara y suelta una flecha naranja de 1,3 daño a 728 u/s',()=>{
 ...
 expect(projectileStats(arrow.classId,arrow.charged)).toMatchObject({speed:728,damage:1.3});
 const q=d.state.players[1];q.x=arrow.x+30;q.y=arrow.y;d.step(new Map());expect(q.hp).toBeCloseTo(1.7);
});
